...
function CouchClient(url) {
  ...
  var saveValues = [];
  var saveQueue = [];
  function realSave(doc, headers, callback) {
    ...
    function consumeQueue() {
      ...
      request("POST", uri.pathname + "/_bulk_docs", {docs: body}, headers, function (err, results) {
        if (results.error) {
  ...
  var getQueue = [];
  var getKeys = [];
  function realGet(key, includeDoc, headers, callback) {
    getKeys.push(key);
    ...
    function consumeQueue() {
      ...
      request("POST", path, {keys: keys}, headers, function (err, results) {
        if (!results.rows) {
          err = new Error("CouchDB Error: " + JSON.stringify(results));
        }
        if (err) {
  ...
  function save(doc, headers, callback) {
    if ( typeof headers == "function" )
      callback = headers, headers = null;
    ...
    if (doc._id) {
      if (!doc._rev) {
        if (!revCache.hasOwnProperty(doc._id)) {
          realGet(doc._id, false, headers, function (err, result) {
            if (err) { return callback(err); }
            if (result) {
              revCache[doc._id] = result.rev;
              doc._rev = result.rev;
            }
            realSave(doc, headers, callback);
          });
          return;
        }
  ...
  function get(key, headers, callback) {
    if ( typeof headers == "function" )
      callback = headers, headers = null;
    realGet(key, true, headers, callback);
  }

  function remove(doc, headers, callback) {
    if ( typeof headers == "function" )
      callback = headers, headers = null;
    ...
    doc._deleted = true;
    save(doc, headers, callback);
  }

  function changes(since, headers, callback) {
    if ( typeof headers == "function" )
      callback = headers, headers = null;
    var stream = request("GET", uri.pathname + "/_changes?feed=continuous&heartbeat=1000&since=" + since, headers);
    var data = "";
    function checkData() {
      var p = data.indexOf("\n");
      if (p >= 0) {
  ...
  function view(viewName, obj, headers, callback) {
    if ( typeof headers == "function" )
      callback = headers, headers = null;
    ...
    request("GET", viewName, headers, callback);
  }
